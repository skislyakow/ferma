...
| Цикл падает на второй итерации | `os.chdir()` в `run_channel()` меняет CWD, относительный `env_path` резолвится неверно | `env_path = os.path.abspath(env_path)` перед циклом |
| Посты-тизеры ("рассказываем о 7 признаках") | Фильтр не ловил листикл-интро без явных external patterns | `TEASER_PATTERNS` — числовые маркеры ("семи", "топ-5", "10 способов") |
